...
export class SvgMap implements OnInit {
  ...
  onMouseEnter(event: MouseEvent): void {
    ...
    hoverElement.style.fill = '#7c4d17';
  ...
  onCountryClick(event: MouseEvent): void {
    ...
    const clickedCountryName = clickedElement.getAttribute('name');

    if (clickedCountryId) {
      // Remove any existing highlight
      const existingHighlight =
        this.svgContainer.nativeElement.querySelector('.country-highlight');
      if (existingHighlight) {
        existingHighlight.remove();
      }

      // Create highlight path
      const highlightPath = clickedElement.cloneNode(true) as SVGPathElement;
      highlightPath.classList.add('country-highlight');
      highlightPath.setAttribute('fill', 'none');
      highlightPath.setAttribute('stroke', '#818581');
      highlightPath.setAttribute('stroke-width', '1.5');

      // Insert the highlight path
      this.svgContainer.nativeElement
        .querySelector('svg')
        .appendChild(highlightPath);
